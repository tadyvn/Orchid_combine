# Usage: 
# Run on single image:
#	python3 eval.py --trained_model=weights/yolact_edge_124_10000.pth --score_threshold=0.3 --top_k=100 --image=img_test/8.jpg --disable_tensorrt

# Run on an input folder (img_test) and save the result at output folder (img_out):
#	python3 eval.py --trained_model=weights/yolact_edge_51_40000.pth --score_threshold=0.3 --top_k=100 --images=img_test:img_out --disable_tensorrt
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    if args.display_masks and cfg.eval_mask_branch:
        ...
        # This is 0 everywhere except for 1 where the mask is
        masks_out = masks
    ...
    for i in range(len(masks_out)):
        if classes[i] == 0: 
            ...
            cv2.imwrite("mask_{}.png".format(i), img_numpy_test) 
    #----------- DRAW SINGLE MASK -----------------------------------#
...
def prep_display_orchid(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    """
    Note: If undo_transform=False then im_h and im_w are allowed to be None.
    """
    if undo_transform:
    ...
    return masks_out, classes, boxes, img_numpy
...
def evalimage(net:Yolact, path:str, save_path:str=None, save_mask:str=None, detections:Detections=None, image_id=None):
    ...
    masks_out, classes, boxes, img_numpy = prep_display_orchid(preds, frame, None, None, undo_transform=False)   
    ...
    if save_path is None:
        ...
        filename = os.path.join(save_mask, "{}.jpg".format(str(name.split('.')[:-1][0]))) 
        print("filename = ", filename)
        cv2.imwrite(filename, img)         
            #----------- DRAW SINGLE MASK -----------------------------------#
        for i in range(len(masks_out)):
            if classes[i] == 0:
                filename = os.path.join(save_mask, "mask_{}.png".format(i))       
                img_numpy_test = (masks_out[i] * 255).byte().cpu().numpy()
                cv2.imwrite(filename, img_numpy_test)
            '''
            elif classes[i] == 2: 
                filename = os.path.join(save_mask, "fullbud_mask.png")       
                img_numpy_test = (masks_out[i] * 255).byte().cpu().numpy()
                cv2.imwrite(filename, img_numpy_test)
            '''
    #----------- DRAW SINGLE MASK -----------------------------------#
...
#***********************************************************************************************************#
# arg.images save operation at this function
#***********************************************************************************************************#
def evalimages(net:Yolact, input_folder:str, output_folder:str, detections:Detections=None):
    ...
    for i, p in enumerate(Path(input_folder).glob('*')):
        ...
        #print("name.split('.')[:-1][0] = ", name.split('.')[:-1][0])
        #print("output_folder = ", output_folder)
        
        folder_name.append(name.split('.')[:-1][0])
        
        # Directory
        temp = str(name.split('.')[:-1][0])
        directory = temp
        ...
        # Path
        output_f = os.path.join(output_folder, directory)
  
        #os.mkdir(output_f, mode)
        if not os.path.exists(output_f):
            os.mkdir(output_f, mode)
            
        out_path = os.path.join(output_f, name)
        #print("out_path = ", out_path)
        
        evalimage(net, path, out_path, output_f,detections=detections, image_id=str(i))
        print(path + ' -> ' + out_path)
    stop = time.time()
