# Usage: 
# Resume training yolact_edge with a specific weight file's name.
#	python3 train.py --config=yolact_edge_config --resume=weights/resnet101_reducedfc.pth 
#	python3 train.py --config=yolact_edge_config --resume=weights/mobilenet_v2-b0353104.pth
#	python3 train.py --config=yolact_edge_config --resume=weights/yolact_edge_54_800000.pth 
#	python3 train.py --config=yolact_edge_config --resume=weights/yolact_edge_64_50000.pth 
#	python3 train.py --config=yolact_edge_config --resume=weights/yolact_edge_12_10000.pth 
...
def train(rank, args):
    ...
    print("seed 1 = ", seed) # 42
    torch.manual_seed(seed)
    np.random.seed(seed) 
    print("seed 2 = ", seed) # 42
    random.seed(seed)
    ...
    import git
    # Failed if uncomment! Check it later
    '''
    ...
    #************************************************************************************
    # COCODetection: --> from data import *
    else:
        print("image path = : ", cfg.dataset.train_images)
        print("corresponding json path = : ", cfg.dataset.train_info)
        dataset = COCODetection(image_path=cfg.dataset.train_images,
                                info_file=cfg.dataset.train_info,
                                transform=SSDAugmentation(MEANS))
        # Only one at the begining
        #print("dataset = / train.py", dataset)
    ...
    #***************************************************************************************************
    if args.resume is not None: # Go with this option (--resume=weights/resnet101_reducedfc.pth)
    ...
    #***************************************************************************************************
        ...
        #print("Go with this option 2") # Go with this option 
    ...
    def backward_and_log(prefix, net_outs, targets, masks, num_crowds, extra_loss=None):
        ...
        losses = criterion(out, targets, masks, num_crowds) # -> check out this function!
        ...
        # Results
        '''
        losses =  {'B': tensor(5.3801, grad_fn=<MeanBackward0>), 'M': tensor(4.6767, grad_fn=<MeanBackward0>), 'C': tensor(3.0428, grad_fn=<MeanBackward0>), 'S': tensor(0.4834, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(5.2598, grad_fn=<MeanBackward0>), 'M': tensor(4.3671, grad_fn=<MeanBackward0>), 'C': tensor(2.8045, grad_fn=<MeanBackward0>), 'S': tensor(0.5540, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(4.9307, grad_fn=<MeanBackward0>), 'M': tensor(4.5246, grad_fn=<MeanBackward0>), 'C': tensor(2.8879, grad_fn=<MeanBackward0>), 'S': tensor(0.4332, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(6.6137, grad_fn=<MeanBackward0>), 'M': tensor(4.4613, grad_fn=<MeanBackward0>), 'C': tensor(2.8888, grad_fn=<MeanBackward0>), 'S': tensor(0.5489, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(5.9971, grad_fn=<MeanBackward0>), 'M': tensor(4.6050, grad_fn=<MeanBackward0>), 'C': tensor(2.8766, grad_fn=<MeanBackward0>), 'S': tensor(0.4436, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(5.5449, grad_fn=<MeanBackward0>), 'M': tensor(4.6180, grad_fn=<MeanBackward0>), 'C': tensor(3.3751, grad_fn=<MeanBackward0>), 'S': tensor(0.3999, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(4.9935, grad_fn=<MeanBackward0>), 'M': tensor(4.3715, grad_fn=<MeanBackward0>), 'C': tensor(3.5301, grad_fn=<MeanBackward0>), 'S': tensor(0.4208, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(4.7127, grad_fn=<MeanBackward0>), 'M': tensor(4.6229, grad_fn=<MeanBackward0>), 'C': tensor(3.2869, grad_fn=<MeanBackward0>), 'S': tensor(0.3644, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(4.7160, grad_fn=<MeanBackward0>), 'M': tensor(4.1281, grad_fn=<MeanBackward0>), 'C': tensor(3.4584, grad_fn=<MeanBackward0>), 'S': tensor(0.3561, grad_fn=<MeanBackward0>)}
        losses =  {'B': tensor(4.3967, grad_fn=<MeanBackward0>), 'M': tensor(4.4834, grad_fn=<MeanBackward0>), 'C': tensor(2.8125, grad_fn=<MeanBackward0>), 'S': tensor(0.4085, grad_fn=<MeanBackward0>)}
        [05/01 08:57:39 yolact.train]: eta: 5 days, 7:57:45  epoch: 0  iter: 20  B: 5.294  M: 4.516  C: 3.140  S: 0.518  T: 13.467  time: 0.573  data_time: 0.004  lr: 0.000136  max_mem: 4144M
        '''
        # Sumary
        # No. of interation: 10
        # Losses: average 
            ...
            while True:
                ...
                if cfg.dataset.name == "FlyingChairs":
                    ...
                    # Go with this option
                        ...
                        #targets: targets (list<tensor>): Ground truth boxes and labels for a batch,
                ...
                if iteration % 10 == 0:
                    ...
                    if torch.cuda.is_available():
                        #print("Pass here!!! Only once every 10 interation. / train.py")
...
def prepare_data(datum):
    ...
    if args.cuda:
        ...
        #print("targets / prepare_data(datum)/ train.py = ", targets)
    #**********************************************************************
    else:
...
if __name__ == '__main__':
    ...
    #print("args.num_gpus = ", args.num_gpus) # args.num_gpus =  1
    if args.num_gpus > 1:
        mp.spawn(train, nprocs=args.num_gpus, args=(args, ), daemon=False)
    else:
        #********************************************************************#
        train(0, args=args) # Run with this option
        #********************************************************************#
