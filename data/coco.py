...
class COCOAnnotationTransform(object):
    ...
    def __call_ORI__(self, target, width, height):
        """
        Args:
    ...
    # Modify here if we want to add other GT value (angle)    
    def __call__(self, target, width, height):
        ...
        for obj in target:
            if 'bbox' in obj:
                ...
                final_box.append(label_idx)
                #final_box.append(angle)
                res += [final_box]  # [xmin, ymin, xmax, ymax, label_idx, angle_idx]
            else:
                print("No bbox found for object ", obj)
        #print("res / ...data/coco.py = ", res)
        return res  # [[xmin, ymin, xmax, ymax, label_idx, angle_idx], ... ]
    #****************************************************************************************************
...
class COCODetection(data.Dataset):
    ...
    def pull_item(self, index):
        ...
        #************************************************************************************************
        # target is created at this point. However, self.transform is not None (Check below) --> target is changed!
        if self.target_transform is not None and len(target) > 0:
            target = self.target_transform(target, width, height)
            #print("target test at self.target_transform/ ...data/coco.py", target)
        #************************************************************************************************

        #************************************************************************************************
        # Go with this option if self.transform is not None (transform=SSDAugmentation(MEANS))
        #************************************************************************************************
        if self.transform is not None:
            #print("Go with this option/ data/coco.py")
            if len(target) > 0:
                target = np.array(target)
                #print("target = / data/coco.py", target)
                ...
                #*********************************************************************************************************
                # MAKE IT CAN BE TRAINED FASTER!!!!!!!!!!!!!!!!!
                #*********************************************************************************************************
                # take the angle GT values to replace on the postion of GT class/ change label from 4 to 5
                #img, masks, boxes, labels = self.transform(img, masks, target[:, :4],
                #    {'num_crowds': num_crowds, 'labels': target[:, 5]})
                #*********************************************************************************************************
            
                # I stored num_crowds in labels so I didn't have to modify the entirety of augmentations
                num_crowds = labels['num_crowds']
                labels     = labels['labels']
                #print("labels = / data/coco.py", labels)
